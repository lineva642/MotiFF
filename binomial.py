...
# def single_motifs_creator_bi(binom_prob, occurrences, args, results_saving_dir, acids=utils.ACIDS_LIST):
...
#     s
#     vector, single = single_motifs_creator_bi(binom_prob, occurrences, args, results_saving_dir, acids=utils.ACIDS_LIST)
...
def get_binom_prob(occurrences, background_n, args, results_saving_dir):
    """
    Parameters
    ----------
    occurrences : 2-d array
        matrix with aa occurences among foreground intervals
    background_n : 2-d array
...
def find_motifs(binom_prob, occurrences, mask, args):
    """
    ...
    """
    aa_df = pd.DataFrame(np.array([np.array(fg_matrix.index)] * fg_matrix.shape[1]).T, index=fg_matrix.index, columns=fg_matrix.columns)
    motifs = []
    motifs_dict = dict()
    mot_length = len(mask)
    for i in aa_df[(binom_prob <= args.p_value) & (occurrences > args.occurrence)].apply(lambda x: x.dropna().to_dict(), axis=1):
        if i:     
            for pos, aa in i.items():
                motifs_dict[pos] = aa
                index = pos + args.interval_length
                mask[index] = aa
                motifs.append([''.join(mask).strip('.'), binom_prob.at[aa, pos], occurrences.at[aa, pos]])
    return motifs, motifs_dict
    


def binomial_alg(occurrences, background_n, args, results_saving_dir):
    final_motifs = []
    motif_l = args.interval_length * 2 + 1
    binom_prob = get_binom_prob(occurrences, background_n, args, results_saving_dir)
    mask = ['.'] * motif_l 
    mask[interval_length] = args.modification_site.lower()
    
    motifs, motif_dict = find_motifs(binom_prob, occurrences, mask, args)
